...
int main(void)
{
	...
	while (1)
	{
		...
		case 2: x = scan_data(RMW); Remove(&s1, x); break;
		case 3: x = scan_data(SCH); idx = IsMember(s1, x);
			printf("s1에 들어 있%s", idx != 0 ? "습니다" : "지 않습니다"); break;
		case 4:
			s1 = s2; break;
		case 5:
			printf("s1==s2=%s\n",s1 == s2 ? "True" : "False");
			...
			printf("s1 | s2 = "); PrintLn(s1 | s2);
			printf("s1 - s2 = "); PrintLn(s1 & ~s2);
			printf("s1 s2 symetticdifference = "); PrintLn((s1 | s2) & ~(s1 & s2));
			break;
		case 6:
			x = scan_data(ADD); Add(&s2, x); break;
		case 7:
			x = scan_data(RMW); Remove(&s2, x); break;
		case 8:
